...
c______for pulse length scaw w/o touching anything else !!
...
c_______ real part and imag part of corr function !
       ...
       sum=(0.d0,0.d0)
       do i=2,nb
       ig=-(0.d0,1.d0)*gg(i)   ! make gamma complex
...
c____________ checking fitted correlation function
       ...
       sum=(0.0,0.0)
       do i=2,nb
       sum=sum+aa(i)*cdexp(ione*gg(i)*time) ! calculate correlation fn.
       enddo
...
c_____coupling system part == will be read from output 

      do i=1,n
      do j=1,n
c      xx(i,j)=dcmplx(0.d0,0.d0)
      xx(i,j)=
      enddo
      enddo
c      xx(2,2)=dcmplx(1.d0,0.d0)
c      xx(4,4)=dcmplx(2.d0,0.d0)   ! four
c      xx(3,3)=dcmplx(

      return
      end

c____________ bath parameters___________
...
c     parameter (twopi=6.28318530717958668d0)
      ...
      include 'poles.f'       ! Change files accordingly for new fitiing
...
c___________ values created in 'poles.f'_________
...
c________gamma and alpha values__________________
      ...
      aa((i-1)*nfit+2)=c(i)*nbeta(beta,zz1)*ione*pi*zz1**2
     .                /

      aa((i-1)*nfit+3)=c(i)*nbeta(beta,-zc1)*ione*pi*(-zc1)**2
     .                /

      aa((i-1)*nfit+4)=c(i)*nbeta(beta,zz2)*ione*pi*zz2**2
     .                /

      aa((i-1)*nfit+5)=c(i)*nbeta(beta,-zc2)*ione*pi*(-zc2)**2
     .                /

      aa((i-1)*nfit+6)=c(i)*nbeta(beta,zz3)*ione*pi*zz3**2
     .              /

      aa((i-1)*nfit+7)=c(i)*nbeta(beta,-zc3)*ione*pi*(-zc3)**2
     .             /


      
      enddo
c________ gamma and alphas for matsubara terms_________
...
c     aa(i)=dcmplx(0.d0,twopi)*wc**3/beta*jjcxx(xx)
       aa(i)=dcmplx(0.d0,twopi)/beta*jjcxx(xx) 
      enddo
...
c==================================================
     ...
     . /(xx-z2(k))/(xx+z2(k))/(xx-dconjg(z2(k)))/(xx+dconjg(z2(k)))
      ...
      do k=1,nfit
      xx=w/wc
c     jjc=jjc+c(k)*wc**3*xx**3
c    . /(xx-z1(k))/(xx+z1(k))/(xx-dconjg(z1(k)))/(xx+dconjg(z1(k)))
c    . /(xx-z2(k))/(xx+z2(k))/(xx-dconjg(z2(k)))/(xx+dconjg(z2(k)))
c     enddo
...
c___________ PULSE PARAMETERS________________
...
c_________________ HAMILTONIAN _______________
...
c________RWA_______________
      ...
      hh(2,2)= (0.d0,0.d0)     
c     hh(3,3)= (0.d0,0.d0)     
c     hh(4,4)=-(omega+2.d0*alpha*time-delta2)
      hh(3,3)=-(omega+2.d0*alpha*time-delta2)
      
      hh(1,2)= e/2.d0*dexp(-(time/tau)**2)
      hh(2,1)= e/2.d0*dexp(-(time/tau)**2)
      hh(2,3)= e/2.d0*dexp(-(time/tau)**2)
      hh(3,2)= e/2.d0*dexp(-(time/tau)**2)
